...
- **容器化执行**: 基于 Apptainer `--containall` 的全隔离执行环境，宿主机环境变量、home、/tmp 均不泄露；支持 Ephemeral Storage 可写层与代理自动透传
...
### 📂 文件传输与代管
- **文件发送/接收**: 通过 SDK/CLI 在本地与服务器之间传输文件和文件夹
- **文件代管 (Custody)**: 将文件托管到后端，获取凭证码供蓝图或其他用户使用
- **自动过期**: 支持 TTL 和下载次数限制
- **容量管控**: 全局容量上限与单文件大小限制
...
│   │   │   ├── __init__.py
│   │   │   ├── execute-mathematica.py                     # Mathematica 执行蓝图
│   │   │   ├── magnus-debug.py                            # Magnus 调试任务蓝图
│   │   │   └── magnus-slurm.py                            # SLURM 任务蓝图
│   │   ├── execute_mathematica.py                         # Mathematica 执行脚本
│   │   ├── fork_config.py                                 # 配置分叉工具
│   │   ├── magnus_debug.py                                # 调试脚本
│   │   ├── magnus_slurm.py                                # SLURM 任务脚本
│   │   ├── migrate_database.py                            # 数据库迁移脚本
│   │   ├── run.py                                         # 脚本执行入口
│   │   ├── pyproject.toml                                 # 脚本依赖配置
│   │   └── tests/                                         # 测试工具
│   │       ├── test_blueprints/                            # Blueprint 测试
│   │       ├── test_github_tools.py
│   │       ├── test_gpu_nvlink.py                         # GPU互联测试
...
│   │   │   ├── [...path]/route.ts                         # API 代理（转发至后端）
│   │   │   └── logo/route.ts                              # Logo API
│   │   ├── globals.css                                    # 全局样式
│   │   ├── icon.png                                       # 应用图标
│   │   ├── layout.tsx                                     # 根布局组件
...
│   │   └── language-context.tsx                           # 国际化语言上下文
│   ├── src/hooks/                                         # 自定义 Hooks
│   │   └── use-job-operations.tsx                         # 任务操作 Hook
│   ├── src/lib/                                           # 工具库
│   │   ├── api.ts                                         # API客户端
│   │   ├── blueprint-defaults.tsx                         # Blueprint 默认模板
│   │   ├── config.ts                                      # 前端配置
│   │   ├── service-defaults.tsx                           # Service 默认模板
│   │   └── utils.ts                                       # 工具函数
...
│       │       ├── actions.py                             # Job/Blueprint 动作
│       │       ├── client.py                              # HTTP 客户端
│       │       ├── config.py                              # SDK 配置管理
│       │       ├── exceptions.py                          # 自定义异常
│       │       ├── file_transfer.py                       # 文件传输
│       │       ├── http_download.py                       # HTTP 下载工具
│       │       └── cli/                                   # CLI 命令行工具包
│       │           ├── __init__.py
...
├── docker/                                                # 容器镜像
│   ├── magnus-runtime/Dockerfile                          # 任务运行时镜像
│   └── slurm-client/Dockerfile                            # SLURM 客户端镜像
├── docs/                                                  # 文档
│   ├── Magnus_SDK_Guide.md                                # SDK & CLI 完整指南
│   ├── Blueprint_Crafting_Guide.md                        # Blueprint 开发指南
│   ├── Magnus_job_runtime.md                              # Job 运行时文档
│   └── about_uv_image.md                                  # UV 镜像文档
├── scripts/                                               # 脚本
│   ├── deploy.py                                          # 部署脚本
│   └── setup_single_node_slurm.sh                         # 单节点 SLURM 部署
└── LICENSE
```
...
server:
  explorer:
    ...
    model_name: "qwen3-235b-a22b"       # 主对话模型
...
server:
  address: http://your-server-ip        # 服务器地址（含协议）
  front_end_port: 3011                  # 前端端口
  back_end_port: 8017                   # 后端端口
  root: /path/to/magnus-data            # 数据存储根目录
  ...
  scheduler:
    ...
    allow_root: false                   # 是否允许 root 用户运行任务
  ...
  file_custody:
    max_size: 10G                       # 代管总容量上限
    max_file_size: 2G                   # 单文件大小上限
    max_processes: 128                  # 最大并发传输数
    default_ttl_minutes: 60             # 默认过期时间（1 小时）
    max_ttl_minutes: 1440               # 最大过期时间（1 天）
...
cluster:
  ...
  max_memory_demand: "256G"             # 最大内存上限
  default_cpu_count: 4                  # 默认 CPU 核心数
  default_memory_demand: "1600M"        # 默认内存限制
  default_runner: "username"            # 默认运行用户
  default_container_image: "docker://pytorch/pytorch:2.5.1-cuda12.4-cudnn9-runtime"
  default_ephemeral_storage: "10G"      # 默认临时存储大小
...
### 文件传输
- `POST /api/files/upload` - 上传文件到服务器代管，返回文件凭证码
- `GET /api/files/download/{token}` - 通过凭证码下载代管文件
...
3. **容器隔离**:
* **`--containall` 全隔离**: 所有任务在 Apptainer `--containall` 模式下运行，宿主机环境变量、`/tmp`、home 目录均不泄露到容器内。
* **Ephemeral Storage**: 每个任务拥有独立的可写覆盖层（sparse overlay，瞬间创建、按需占磁盘），任务结束后自动销毁，容器文件系统默认只读。
* **代理自动透传**: 宿主机的代理环境变量（`HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 等）自动传入容器，桥接模式下 `127.0.0.1` 自动转译为网关地址。
* **网络模式**: 默认使用宿主机网络（host 模式）；可通过 `MAGNUS_NET_MODE=bridge` 切换为 rootlesskit 桥接模式，容器获得独立网络命名空间，通过 `MAGNUS_PORT_MAP` 声明端口映射。
...
> 📖 **完整文档**: [Magnus SDK & CLI 完整指南](docs/Magnus_SDK_Guide.md) 包含所有 API 详解、异步支持、CLI 命令等。另见 [Blueprint 开发指南](docs/Blueprint_Crafting_Guide.md)。
...
**输出示例**：
```
  MAGNUS_ADDRESS  http://your-server-ip:8017
...
9. **文件传输与代管**: 支持文件发送/接收、服务器端代管（Custody），带 TTL 和下载次数限制。
10. **国际化 (i18n)**: 中英双语支持，运行时动态切换。
11. **资源生命周期管理**: 容器镜像与代码仓库的 LRU 缓存淘汰策略。
